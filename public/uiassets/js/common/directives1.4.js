...
myApp.service("validations", function () {
    this.formats = {
        ...
        rates: /^(0|0.25|0.1|0.10|1|1.5|1.50|3|5|7.5|7.50|12|18|28)$/,
    ...
    function ($q, $parse, $http, $sce, $timeout, $templateCache, $interpolate, $window, shareData, $location) {
        ...
        function link(scope, elem, attrs, ctrl) {
            ...
            function keyupHandler(event) {
                ...
                if (which === KEY_UP || which === KEY_EN) {
                    ...
                    else if (!pattern.test(scope.searchStr) && scope.searchStr.length >= Math.max(4, parseInt(minlength))) {
                        initResults();
                        if (searchTimer) {
                            $timeout.cancel(searchTimer);
                        }
            ...
            scope.inputChangeHandler = function (str) {
                ...
                else if (str && !pattern.test(str) && str.length < Math.max(4, minlength)) {
        ...
        return {
            ...
            template: '<div class="autocomplete-holder" data-ng-mouseup="mouseup($event)" data-ng-mousedown="mousedown($event)">' +
                '  <input id="{{id}}_value" name="{{inputName}}" data-ng-model="searchStr" type="text" placeholder="{{placeholder}}"' +
                'data-ng-blur="hideResults($event)"  data-ng-focus="focusHandler()" maxlength="{{maxlength}}" minlength="{{minlength}}" class="form-control" autocapitalize="off" autocorrect="off" autocomplete="off" data-ng-change="inputChangeHandler(searchStr)" data-ng-disabled="disableAfterSelect==\'true\' && searchStr.length > 0" data-ng-class="{\'hasclear\':disableAfterSelect}"/><span data-ng-if="disableAfterSelect==\'true\' && searchStr.length > 0" data-ng-click="resetField()" class="clearer fa fa-close"></span>' +
                '<div ng-if="descError"><span data-ng-bind="descErrorMsg" class="err"></span></div>' +
                '  <div id="{{id}}_dropdown" class="autocomplete-dropdown" ng-show="showDropdown" ng-class="{\'dpdwn\' : !showViewAll && results.length>5 }" style="width: 400px">' +
                '    <div class="autocomplete-searching" ng-show="searching" ng-bind="textSearching"></div>' +
                '    <div class="autocomplete-searching" ng-show="!searching && (!results || results.length == 0)" ng-bind="textNoResults"></div>' +
                '    <div class="autocomplete-row" ng-repeat="result in results | limitTo: showViewAll ? 5 : results.length" ng-click="selectResult(result)" ' +
...
myApp.directive('mstrAutocompleteHsn', ['$q', '$parse', '$http', '$sce', '$timeout', '$templateCache', '$interpolate', '$window', 'shareData', '$location',
    function ($q, $parse, $http, $sce, $timeout, $templateCache, $interpolate, $window, shareData, $location) {
        ...
        return {
            ...
            template: '<div class="autocomplete-holder" data-ng-mouseup="mouseup($event)" data-ng-mousedown="mousedown($event)">' +
                '  <input id="{{id}}_value" name="{{inputName}}" data-ng-model="searchStr" type="text" placeholder="{{placeholder}}"' +
                'data-ng-blur="hideResults($event)" required  data-ng-focus="focusHandler()" maxlength="{{maxlength}}" minlength="{{minlength}}" class="form-control" autocapitalize="off" autocorrect="off" autocomplete="off" data-ng-change="inputChangeHandler(searchStr)" data-ng-disabled="disableAfterSelect==\'true\'" data-ng-class="{\'hasclear\':disableAfterSelect}"/><span data-ng-if="disableAfterSelect && searchStr.length > 0" data-ng-click="resetField()" class="clearer fa fa-close"></span>' +
