...
window.constants = {
	...
	"EM_PORTS" :  "1645, 8080, 1812, 2083, 2948",
...
(function ($) {
    'use strict';
    var wh = 0, fs = 0, cfs, it = 0, t, CONSTANTS = {}, timerInt, winDigest = true, previousScroll = 0, caretTimeoutId, ua = navigator.userAgent, iPhone = /iphone/i.test(ua), chrome = /chrome/i.test(ua), android = /android/i.test(ua);
    $(document).on("click", ".datepicker-icon span", function () {
        //$(this).parent().find("input").focus();
        $(this).parent().find("input").trigger("openDPicker");
    });
    ...
    $.fn.reposFooter = function () {
        ...
        } else if ($(".content-wrapper .container-fluid").length > 0) {
			if ($(".content-wrapper .container-fluid").height() <= $(window).height() * 0.6) {
                wh = $(window).height() * 0.6;
            }
			//$(".content-wrapper .container-fluid").css("min-height", wh);
		}
    };
    $().reposFooter();
    ...
    $(document).on('click', 'a[data-popup]', function (ev) {
        ev.preventDefault();
        var href = $(this).attr('href'), content = "This screen shall take you to a web page outside GST Portal.</p><p> For any queries regarding the content of the linked page, please contact the webmaster of the concerned website.";
		if($(this).attr('data-download') == 'true') {
			content = "Are you sure you want to download this utility?"
		}
		$("#dataConfirmModal").remove()
        
		$('body').append('<div id="dataConfirmModal" class="modal fade" role="dialog" aria-labelledby="dataConfirmLabel"><div class="modal-dialog sweet"><div class="modal-content"><div class="modal-body"><div class="m-icon m-warning pulseWarning"><span class="micon-body pulseWarningIns"></span><span class="micon-dot pulseWarningIns"></span></div><h2>Information</h2><p>'+content+'</p></div><div class="modal-footer"><a class="btn btn-default" data-dismiss="modal">Cancel</a><a class="btn btn-primary" id="dataConfirmOK" target="_blank" >Proceed</a></div></div></div></div>');
   ...
   //      }
	...
	$(".high-low").on("click", function () {
		if (!$("link[title=app-css]").hasClass("inverted")) {
			$("link[title=app-css]").toggleClass("inverted").attr("href", $("link[title=app-css]").attr('href').replace("/app", "/iapp"));
			$().setCookie("CC", true, 15);
		} else {
			$("link[title=app-css]").toggleClass("inverted").attr("href", $("link[title=app-css]").attr('href').replace("/iapp", "/app"));
