...
(function () {
    ...
    var monthPattern = /^(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER)$/,
        yearPattern = /^(2017-18|2018-19|2019-20|2020-21|2021-22)$/;
    ...
    g1FileHandler.$inject = ["$q", "$http", "$filter"];

    function g1FileHandler(Q, Http, Filter) {
        return {
        ...
        function getDropdown() {
            var deferred = Q.defer(),
                ...
                MM = DD.getMonth(),
                YYYY = DD.getFullYear(),
                nbrOfYearsToDisplay = (YYYY <= "2018") ? 2 : 4;

            function formateMMasNN(iNum) {
                return (Math.floor(iNum / 10)) ? iNum : "0" + iNum;
            }

            for (var i = YYYY, iEnd = (YYYY - nbrOfYearsToDisplay); i > iEnd; i--) {
        ...
        // Product Master 
        function saveProductMstr(gstin, savemstr, recType) {
            var deferred = Q.defer(),
                iParam = {
                    "savemstr": savemstr,
                    "gstin": gstin,
        ...
        function getContentsForMeta(d, iSectionName, fSent) {
            ...
            var form = d.form;
            Http.post('/fetchMeta', { fName: fName, form: form }).success(function (response) {
                if (iSectionName) {
    ...
    function R1Util($compile, utilFunctions, shareData, timeout) {
        ...
        function onRtChange(d, e) {
            ...
            if (shareData.dashBoardDt.tbl_cd === 'b2cs') {
                if (d.diff_percent) {
            ...
            if (supGstin && form == "GSTR1") {
                if (iInv.pos) {
                    var ctin = supGstin.slice(0, 2),
                        pos = iInv.pos;
                    if (isSEZ) {
                        if (tableCode === 'b2cs' || tableCode === 'b2csa' || tableCode === 'at' || tableCode === 'atadj' || tableCode === 'ata' || tableCode === 'atadja') {
                            iInv.sply_ty = "INTER";
                        }
                        else {
    ...
    function R1InvHandler($log, Q, R1Util, shareData, g1FileHandler, Filter, timeout, validations, Http) {
        ...
        function onPasteErrorHandler($scope, response, iErrInvList, iMatchedErrList, iPatternErrList, iTaxRtErrList, iShipDtErrList, iActionErrList, URTypPosErrList, UrTypDiffErrList, PosSupStCdErrList, NoteSplyTpeErrList, iErrorStatusList, isHeaderMissingList) {
            if ((Array.isArray(response) && response.length) || (Array.isArray(iErrInvList) && iErrInvList.length) || (Array.isArray(iMatchedErrList) && iMatchedErrList.length)
                ...
                || (Array.isArray(NoteSplyTpeErrList) && NoteSplyTpeErrList.length)) {
        ...
        function upMultipleSections($scope, iJSON, iErrInvList, iMatchErrList, iPatternErrList, iTxRtErrList, iShipDtErrList, iActionErrList, iErrorStatusList, isHeaderMissingList, iFlag, URTypPosErrList, UrTypDiffErrList, PosSupStCdErrList, NoteSplyTpeErrList) {
            var deferred = Q.defer(),
            ...
            if (stdata.length || iJSON.cache_key !== undefined) {
                ...
                g1FileHandler.saveMultiplePayload(reqParam)
                    .then(function (response) {
                        ...
                        onPasteErrorHandler($scope, response, iErrInvList, iMatchErrList, iPatternErrList, iTxRtErrList, iShipDtErrList, iActionErrList, URTypPosErrList, UrTypDiffErrList, PosSupStCdErrList, NoteSplyTpeErrList, iErrorStatusList, isHeaderMissingList);
                        if (iFlag == 'E') {
        ...
        function getNoteSupplyType(isec, invtype) {
            if (isec == 'cdnr' || isec == 'cdnra' || isec == 'b2b' || isec == 'b2ba') {
                ...
                else if (invtype.trim() == 'Regular') return "R";
        ...
        function preparePayloadFromExcel(oData, getInvFn, getItmFn, excelRefKey, newFormateKey, iSecID, iForm, iMonthsList, iYearsList, iSpLs, supplier_gstin, isSEZ, typee, transLan) {
            if (!typee)
                typee = 'copy';
            var iData = null;
            var invAry = [];
            ...
            iData.forEach(function (inv, i) {
                ...
                var curInum = inv[excelRefKey],
                    isValidExcelFields = validateExcelMandatoryFields(inv, iSecID, iForm, transLan, supplier_gstin),
    ...
    function ReturnStructure($log, Q, R1Util, shareData, g1FileHandler, Http) {
        ...
        function getInvType(isec, invtype) {
            if (isec == 'b2b' || isec == 'b2ba' || isec == 'cdnr' || isec == 'cdnra') {
